...
function notification_decode ($type,$owner) {
	global $type_name,$message,$skip,$ale,$values,$system_sql,$moon_sql,$evedb;
	...
	switch ($type) {
		...
		case '41':
			...
			*/
			...
			$message = "$corp_name has lost sovereignty in $system_name";
			break;
		case '42':
		...
		case '45':
			...
			if (array_key_exists ( 'moonID' , $values ))
			{
				...
				$evedb->moon_sql->execute(array(':moonid'=>$moonid));
				while ($moon_row=$evedb->moon_sql->fetch()){
		...
		case '48':
			...
			*/
			...
			if (array_key_exists ( 'corpID' , $values ))
			{
				...
				$corp_details = $ale->corp->CorporationSheet($params2);
				$corp_name=$corp_details->result->corporationName;
				if (!empty($corp_details->result->allianceName))
				{
					$alliance_name=$corp_details->result->allianceName;
				}
			}
			...
			$message = "$corp_name of $alliance_name is anchoring SBUs in $system_name";
		...
		case '75':
			...
			aggressorCorpID: 98044748
			aggressorID: 341058425
			armorValue: 1.0
			hullValue: 1.0
			-moonID: 40275345
			shieldValue: 0.99232
			-solarSystemID: 30004347
			typeID: 27897
			...
			*/
			...
			if (array_key_exists ( 'shieldValue' , $values ))
                        {
				$shield = number_format((float)$values['shieldValue']*100, 2, '.', '');
			}

			if (array_key_exists ( 'armorValue' , $values ))
                        {
				$armor = number_format((float)$values['armorValue']*100, 2, '.', '');
			}
			
			if (array_key_exists ( 'hullValue' , $values ))
                        {
				$hull = number_format((float)$values['hullValue']*100, 2, '.', '');
			}
			...
			$message = "Tower (Owner:$owner) on moon $moon_name in $system_name is under attack by $corp_name from $alliance_name\nStatus: \nShield - $shield\nArmor - $armor\nHull - $hull";
		...
		case '76':
			...
			*/
			...
			$message = "POS on moon $moon_name (owned by $corp_name) in $system_name is low on resources";
		...
		case '77':
			...
			*/			
			...
			$message = "Station in $system_name is under attack by $corp_name from $alliance_name\nStatus: \nShield - $shield\nArmor - $armor\nHull - $hull";
		...
		case '86':
			...
			$message = "TCU in $system_name (Owner:$owner) is under attack by $corp_name from $alliance_name\nStatus: \nShield - $shield\nArmor - $armor\nHull - $hull";

			//$message = "$type_name in $system_name under attack by $corp_name from $alliance_name";
			break;
		case '87':
			...
			*/
			...
			$message = "SBU in $system_name (Owner:$owner) is under attack by $corp_name from $alliance_name\nStatus: \nShield - $shield\nArmor - $armor\nHull - $hull";
		...
		case '88':
			...
			$message = "iHub in $system_name (Owner:$owner) is under attack by $corp_name from $alliance_name\nStatus: \nShield - $shield\nArmor - $armor\nHull - $hull";
