...
$l = fopen("/home/barney/Notification_Broadcast/eh.txt", "a");
...
class evedb {
	...
	function __construct()
	{
		...
		$this->api_count_sql = $this->pdo->prepare('select count(*) as count from api_keys where corp = "EH"');
		$this->previous_sql = $this->pdo->prepare('select notificationID from notification where notificationID= :nid');
		$this->api_sql = $this->pdo->prepare('select id,key_id,vcode,charid from api_keys where corp = "EH"');
...
$jabcon = new XMPPHP_XMPP($config['jabber']['host'], 5222, $config['jabber']['user'], $config['jabber']['password'], 'Notifier-EH','lawnalliance.org',$printlog=true, $loglevel=XMPPHP_Log::LEVEL_DEBUG);
			...
			foreach (array_keys($params) as $group)
			{
				...
				if ($params[$group]['IDs']!='')
				{
					...
					foreach ($row->rowset->row as $notice)
					{
						...
						notification_decode($notifications[$group][(int)$notice->attributes()->notificationID]['type'],"Entwurf Hausfrau");
