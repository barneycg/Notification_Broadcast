...
$api_count_sql = $pdo->prepare('select count(*) as count from api_keys where corp = "BLUEP"');
$previous_sql = $pdo->prepare('select notificationID from notification where notificationID= :nid');
$api_sql = $pdo->prepare('select id,key_id,vcode,charid from api_keys where corp = "BLUEP"');
...
$jabcon = new XMPPHP_XMPP($config['jabber']['host'], 5222, $config['jabber']['user'], $config['jabber']['password'], 'Notifier-LANWS');
