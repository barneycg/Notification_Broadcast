...
$jabcon = new XMPPHP_XMPP($config['jabber']['host'], 5222, $config['jabber']['user'], $config['jabber']['password'], 'Notifier-LOON','lawnalliance.org',$printlog=true, $loglevel=XMPPHP_Log::LEVEL_DEBUG);
			...
			foreach (array_keys($params) as $group)
			{
				...
				if ($params[$group]['IDs']!='')
				{
					...
					foreach ($row->rowset->row as $notice)
					{
						...
						notification_decode($notifications[$group][(int)$notice->attributes()->notificationID]['type'],"LAWN Moons");
